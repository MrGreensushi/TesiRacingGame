...
def single_care_dataframe(path):
    df_gara=pd.read_csv(path, sep=";", header=None , decimal=',',names=["Player", "X", "Z", "VEL_X","VEL_Z","ROT"])
...
def recreate_dataframe(series):
    columns=["DIFF_X","DIFF_Z","DIFF_VEL_X","DIFF_VEL_Z","DIFF_ROT","RACE"] 
...
def Generator(df):
    grouped=df.groupby("RACE").apply(batch_generator)
    for group in grouped:
        for single in group:
            yield single
            
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self,batch_size,df,max_batch,mean,std):
    ...
    def __getitem__(self,index):
        ...
        for i in range(self.batch_size):
            ...
            #        break
            x,y=next(self.generator)
            y=y*self.std+self.mean # since the data has been normalized i want the target to be not normalized
            X.append(x)
            Y.append(y)
