...
def batch_generator(df):
    ...
    dropped_df=df.drop(["Player","RACE"],axis=1).reset_index(drop=True)
...
def single_care_dataframe(path):
    ...
    df_gara_stacked=pd.DataFrame(lst_df, columns=["Player","VEL_X","VEL_Z","X","Z","ROT","RACE"])
        
    return df_gara_stacked

def subtraction_columns(df):
    df_copy=df.shift(1,fill_value=0)
    cols = df.columns.difference(['Player','RACE'])
...
def get_split(x,first,second):
    ...
    if DIFF_ROT:
        #df_gara_stacked["ROT1"]= df_gara_stacked.ROT.shift(1,fill_value=0)
        #df_gara_stacked["DIFF_ROT"]=(df_gara_stacked["ROT1"]-df_gara_stacked["ROT"] + 180)%360-180
        x=subtraction_columns(x)
        x["ROT"]=(x["ROT"]+180)%360-180
        #(a + 180) % 360 - 180
    else:
        x.drop("ROT",axis=1,inplace=True)
...
def recreate_dataframe(series):
    if DIFF_ROT:
        columns=["Player","DIFF_VEL_X","DIFF_VEL_Z","DIFF_X","DIFF_Z","DIFF_ROT","RACE"]
    else:
        columns=["Player","VEL_X","VEL_Z","X","Z","RACE"]  
...
def Generator(df):
    grouped=df.groupby(["Player","RACE"]).apply(batch_generator)
