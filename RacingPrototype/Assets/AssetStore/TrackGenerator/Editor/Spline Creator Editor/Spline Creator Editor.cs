...
public class SplineCreatorEditor : Editor
{
    ...
    private void GeneralControl()
    {
        EditorGUILayout.BeginVertical();
        {
            ...
            sc.CheckPoint = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Online Checkpoint",
                                                          "Checkpoint gameobject to be instantiated"),
                                                          sc.CheckPoint, typeof(GameObject), true);
            //Offline checkpoint
            sc.CheckPointOffline = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Offline Checkpoint",
                                                         "Checkpoint gameobject to be instantiated"),
                                                         sc.CheckPointOffline, typeof(GameObject), true);

            //Wall Gameobject
            sc.Wall = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Wall Gameobject",
                                                          "Wall gameobject to be instantiated"),
                                                          sc.Wall, typeof(GameObject), true);

            sc.Offline = EditorGUILayout.Toggle(new GUIContent("Offline ",
                "The track shoub be used for offline content or not"), sc.Offline);

            sc.NumberOfGates = EditorGUILayout.IntField(new GUIContent("Number of Gates",
                                                       "The total number of gates to be instantiated in the track."),
                                                         sc.NumberOfGates);
        }
            ...
            if (sc.Spline.ControlPointsModifiable)
            {
                if (sc.Spline.SplineCurveType == CurveType.Bezier)
                {
                    for (int i = 0; i < sc.Spline.ControlPoints.Count; i += 3)
                    {
                        ...
                        var fmh_405_65_638428150611087967 = Quaternion.identity; Vector3 newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i], 8f, Vector3.zero,
                        ...
                        var fmh_416_65_638428150611110216 = Quaternion.identity; newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i + 2], 8f, Vector3.zero,
                                                                Handles.CylinderHandleCap);
                        //if newPos is not the same as the control point on the spline 
                        ...
                        //Draw the site point
                        Handles.color = Color.red;
                        var fmh_430_65_638428150611113074 = Quaternion.identity; newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i + 1], 8f, Vector3.zero,
                                                                Handles.CylinderHandleCap);
                        //if newPos is not the same as the control point on the spline 
                ...
                else if (sc.Spline.SplineCurveType == CurveType.CatmullRom)
                {
                    for (int i = 0; i < sc.Spline.ControlPoints.Count; i++)
                    {
                        //Draw the start and end control point
                        Handles.color = Color.red;
                        var fmh_458_65_638428150611116606 = Quaternion.identity; Vector3 newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i], 8f, Vector3.zero,
                                                                Handles.CylinderHandleCap);
                        //if newPos is not the same as the control point on the spline 
                        if (sc.Spline.ControlPoints[i] != newPos)
                        {
