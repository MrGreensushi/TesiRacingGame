...
public class SplineCreator : MonoBehaviour
{
    //Added by me
    [SerializeField]
    private GameObject _checkPoint, _checkPointOffline, _wall;
    private Transform _checPointsTransform;
    private Transform _wallsTransform;
    private bool _offline;
    private int _numberOfGates;

    public int NumberOfGates { get { return _numberOfGates; } set { _numberOfGates = value; } }
    public bool Offline { get { return _offline; } set { _offline = value; } }
    public GameObject CheckPoint { get { return _checkPoint; } set { _checkPoint = value; } }
    public GameObject CheckPointOffline { get { return _checkPointOffline; } set { _checkPointOffline = value; } }
    public GameObject Wall { get { return _wall; } set { _wall = value; } }
    /*
     * Spline
    ...
    */
    ...
    private void CreateTrackVertices()
    {
        ...
        _checPointsTransform = parent.transform;
        _wallsTransform = walls.transform;

        //get size of wall
        var render = _wall.GetComponent<Renderer>();
        var width = render.bounds.size.z;
        ...
        //I want a total of 40 checkpoint
        int nChecks = oPoints.Length / _numberOfGates;
        ...
        for (int i = 0; i < oPoints.Length; i++)
        {
            ...
            if (i == 0)
            {
                GameObject sl;
                if (_offline)
                    sl = Instantiate(_checkPointOffline, oPoints[i].Position,
                   oPoints[i].Rotation);
                else
                    sl = Instantiate(_checkPoint, oPoints[i].Position,
                   oPoints[i].Rotation);

                sl.transform.Rotate(0, 90, 0);
                sl.transform.parent = _checPointsTransform;
            }
            //every nChecks oPoints I instatiate a checkPoint
            if ((i % nChecks) == 0)
            {
                if (i != 0)
                {
                    GameObject checki;
                    if (_offline)
                        checki = Instantiate(_checkPointOffline, oPoints[i].Position,
                       oPoints[i].Rotation);
                    else
                        checki = Instantiate(_checkPoint, oPoints[i].Position,
                       oPoints[i].Rotation);

                    checki.transform.Rotate(0, 90, 0);
                    checki.transform.parent = _checPointsTransform;
                }
            }
            ...
            for (int j = 0; j < pTwoDShape.Vertices.Count; j++)
            {
                ...
                if (i == 0)
                    continue;
                var previous = proMesh.Vertices[i * 2 + j - 2];
                var distance = Mathf.Abs(Vector3.Distance(previous, pos));
                var rotation = Quaternion.LookRotation(Vector3.Cross((pos - previous), Vector3.up), Vector3.up);
                var zScale = distance / width;
                GameObject wall = Instantiate(_wall, (previous + pos) / 2, rotation, _wallsTransform);
        ...
        //Wall for instantiate i=0
        for (int j = 0; j < 2; j++)
        {
            var previous = proMesh.Vertices[proMesh.Vertices.Count - 2 + j];
            var actual = proMesh.Vertices[j];
            var distance = Mathf.Abs(Vector3.Distance(previous, actual));
            var rotation = Quaternion.LookRotation(Vector3.Cross((actual - previous), Vector3.up), Vector3.up);
            var zScale = distance / width;
            GameObject wall = Instantiate(_wall, (previous + actual) / 2, rotation, _wallsTransform);
            wall.transform.Rotate(0, 90, 0);
            wall.transform.localScale = new Vector3(1, 1, zScale);
        }
    ...
    private void GenerateTrackObject()
    {
        ...
        if (trackMesh != null && curbsMesh != null)
        {
            ...
            GameObject pista = new GameObject("Pista");
            pista.transform.SetParent(track.transform);
            pista.AddComponent<MeshFilter>();
            pista.AddComponent<MeshRenderer>();
            pista.AddComponent<MeshCollider>();

            //set the mesh filter and mesh renderer
            pista.GetComponent<MeshFilter>().sharedMesh = trackMesh;
            pista.GetComponent<MeshCollider>().sharedMesh = trackMesh;
            pista.GetComponent<MeshRenderer>().sharedMaterial = TrackMaterial;

            //then create a new GameObject add a mesh filter and mesh renderer to it
            GameObject curb = new GameObject("Cars");
            curb.transform.SetParent(track.transform);
            curb.AddComponent<CarHolder>();

            //Set track as checpoints parent
            _checPointsTransform.SetParent(track.transform);
            _wallsTransform.SetParent(track.transform);
            if (!_offline) return;

            //create laps and car manages
            GameObject managers = new GameObject("Managers");
            managers.transform.SetParent(track.transform);
            var carM = managers.AddComponent<Offline_CarsManager>();
            var lapM = managers.AddComponent<Offline_LapsManager>();
            lapM.CarsManager = carM;
            lapM.SetFindGates = true;
            lapM.InitializeIds = true;
            carM.LapsManager = lapM;
            lapM.ParentGates = _checPointsTransform;
        }
    }
