...
namespace QuickStart
{
    public class LapsManager : NetworkBehaviour
    {
        ...
        public void CarPassedThrough(int id_gate, string id_car)
        {
            ...
            //Gate indica il prossimo Gate da passare, quindi controllo se il gate in cui � passato � lo stesso di quello che doveva passare
            if (c.Gates == id_gate)
            {
                c.Gates = id_gate + 1;
            ...
            //se il valore di gate � maggiore del numero di gate in campo vuol dire che � passato in tutti i gate
            //se � passato per tutti i gates e ora sta passando al gate iniziale ha fatto un giro
        ...
        private int GetGateIdPlayer(string id_car)
        {
            ...
            {
                Debug.LogWarning($"Car {id_car} is not in the server cars list");
                //2 possibilit� o c'� stato un errore oppure il codice per aggiungere la macchina nel carmanager non � stato ancora eseguito
                return -1;
            }
            CarDescriptor c = CarsManager.instance.cars[passed].Car;
